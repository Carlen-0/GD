...
%         figure(1);hold on;title(file);
    ...
    %% 写曲线数据包
    if transform_open == 1
        path_Excel = 'F:\项目_2025\ZC08E\采集数据\微距镜头\7_High.xlsx';
        [path_excel,name_excel,ext_excel] = fileparts(path_Excel);
%         if ~exist([path '\new'],'dir')
%             mkdir([path '\new']);
%         end
		if ~exist(name_excel)
        ...
        [a,b,c] = xlsread(path_Excel);
        
        usCurveTemperatureNumber = 7000;
        ucFocusNumber = 6;
        ...
        %标准镜1；广角7；中长焦8；长焦9；微距10；高温11；
        %双视场镜头（标准+中长焦）—标准镜65；双视场镜头（标准+中长焦）—中长焦96；
        %双视场镜头（标准+广角）—标准镜64；双视场镜头（标准+广角）—广角80；
        fFieldType = 10;   
        switch fFieldType
            case 1
                newcurve_name = 'ita_standard_high.raw';
                sDistanceArray = [15;30;0;0;0];
            case 7
                newcurve_name = 'ita_wideangle_high.raw';
                sDistanceArray = [15;30;0;0;0];
            case 8
                newcurve_name = 'ita_tele_high.raw';
                sDistanceArray = [50;50;0;0;0];
            case 9
                newcurve_name = 'ita_85_tele_high.raw';
                sDistanceArray = [50;50;0;0;0];
            case 10
                newcurve_name = 'ita_micro_high.raw';
                sDistanceArray = [1;1;0;0;0];
            case 65
                newcurve_name = 'ita_std_mtele_high.raw';
                sDistanceArray = [15;30;0;0;0];
            case 96
                newcurve_name = 'ita_mtele_std_high.raw';
                sDistanceArray = [50;50;0;0;0];
            case 64
                newcurve_name = 'ita_std_wide_high.raw';
                sDistanceArray = [15;30;0;0;0];
            case 80
                newcurve_name = 'ita_wide_std_high.raw';
                sDistanceArray = [15;30;0;0;0];    
        end
        
        fid = fopen([[path '/' name_excel] '/' newcurve_name], 'w');
       
%         [a,b,c] = xlsread('D:\项目\ZC08E\原理样机_双视场镜头（标准+广角）\4_标准_High.xlsx');
        ...
        %5℃环温对应曲线
        Data_5(:,1)=[0.2568 19.3013 33.6826 48.1185 96.4470 144.8938 193.3441 290.1154 386.6651 434.8596 627.1877];    %High 
        Data_5(:,2)=(m_data(1,end-10:end))';
        Data_5(:,2) = Data_5(:,2) - Data_5(1,2) + 1000;
        nCurve(2,:) = Curve_High(Data_5);
        
        %15℃环温对应曲线
        Data_15(:,1)=[0.8111 19.7557 34.0812 48.4729 96.7061 145.1006 193.5190 290.2552 386.7866 434.9751 627.2891];
        Data_15(:,2)=(m_data(2,end-10:end))';
        Data_15(:,2) = Data_15(:,2) - Data_15(1,2) + 1000;
        nCurve(3,:) = Curve_High(Data_15);
        
        %23℃环温对应曲线
        Data_23(:,1)=[1.2937 20.1521 34.4292 48.7826 96.9327 145.2815 193.6722 290.3775 386.8930 435.0762 627.3778];
        Data_23(:,2)=(m_data(3,end-10:end))'; 
        Data_23(:,2) = Data_23(:,2) - Data_23(1,2) + 1000;
        nCurve(4,:) = Curve_High(Data_23);
        
        %35℃环温对应曲线
        Data_35(:,1)=[2.0827 20.8014 35.0000 49.2908 97.3053 145.5792 193.9243 290.5789 387.0682 435.2428 627.5239];
        Data_35(:,2)=(m_data(4,end-10:end))';
        Data_35(:,2) = Data_35(:,2) - Data_35(1,2) + 1000;
        nCurve(5,:) = Curve_High(Data_35);
