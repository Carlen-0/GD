...
curve_name = 'ita_standard_low.raw';
...
%         figure(1);hold on;title(file);
    ...
    %% 写曲线数据包
    if transform_open == 1
        path_Excel = 'F:\项目_2025\ZC08E\采集数据\微距镜头\7_Low.xlsx';
...
%         end
        ...
        switch fFieldType
            case 1
                newcurve_name = 'ita_standard_low.raw';
                sDistanceArray = [15;30;0;0;0];
            case 7
                newcurve_name = 'ita_wideangle_low.raw';
                sDistanceArray = [15;30;0;0;0];
            case 8
                newcurve_name = 'ita_tele_low.raw';
                sDistanceArray = [50;50;0;0;0];
            case 9
                newcurve_name = 'ita_85_tele_low.raw';
                sDistanceArray = [50;50;0;0;0];
            case 10
                newcurve_name = 'ita_micro_low.raw';
                sDistanceArray = [1;1;0;0;0];
            case 65
                newcurve_name = 'ita_std_mtele_low.raw';
                sDistanceArray = [15;30;0;0;0];
            case 96
                newcurve_name = 'ita_mtele_std_low.raw';
                sDistanceArray = [50;50;0;0;0];
            case 64
                newcurve_name = 'ita_std_wide_low.raw';
                sDistanceArray = [15;30;0;0;0];
            case 80
                newcurve_name = 'ita_wide_std_low.raw';
                sDistanceArray = [15;30;0;0;0];
        end
...
%         [a,b,c] = xlsread('D:\项目\ZC08E\原理样机_双视场镜头（标准+广角）\4_标准_Low.xlsx');
        ...
        %5℃环温对应曲线
        Data_5(:,1)=[-9.1832 0.2568 19.3013 33.6826 48.1185 96.4470 144.8938];    %Low
        Data_5(:,2)=(m_data(1,end-6:end))';
        Data_5(:,2) = Data_5(:,2) - Data_5(2,2) + 3000;
        nCurve(2,:) = Curve_Low(Data_5);
        
        %15℃环温对应曲线
        Data_15(:,1)=[-8.5638 0.8111 19.7557 34.0812 48.4729 96.7061 145.1006];
        Data_15(:,2)=(m_data(2,end-6:end))';
        Data_15(:,2) = Data_15(:,2) - Data_15(2,2) + 3000;
        nCurve(3,:) = Curve_Low(Data_15);
        
        %23℃环温对应曲线
        Data_23(:,1)=[-8.0252 1.2937 20.1521 34.4292 48.7826 96.9327 145.2815];
        Data_23(:,2)=(m_data(3,end-6:end))';
        Data_23(:,2) = Data_23(:,2) - Data_23(2,2) + 3000;
        nCurve(4,:) = Curve_Low(Data_23);
        
        %35℃环温对应曲线
        Data_35(:,1)=[2.0827 20.8014 35.0000 49.2908 97.3053 145.5792];
        Data_35(:,2)=(m_data(4,end-5:end))';
        Data_35(:,2) = Data_35(:,2) - Data_35(1,2) + 3000;
        nCurve(5,:) = Curve_Low(Data_35);
