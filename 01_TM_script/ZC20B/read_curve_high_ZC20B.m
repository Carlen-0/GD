...
filelist = dir([path '\*_High.raw']);   % 高温对应High，低温对应Low
...
for file = 1:length(filelist)
    ...
    %% 写曲线数据包
    if transform_open == 1
        if ~exist([path '/high'],'dir')
            mkdir([path '/high']);
        end
        fid = fopen([path '/high/' filelist(file).name], 'w');
        % 更改项
       ...
       % fwrite(fid, zeros(1, (24-length(cModuleCode))), 'char');
        ...
        for i = 1:usFocusArrayLength/2
            if i == 1
                curve_distance1(i, :) = curve_data_distance1(1, :) + ...
                    (usFocusArray(1)-usFocusArray(2))/(usFocusArray(2)-usFocusArray(3))*(curve_data_distance1(1,:) - curve_data_distance1(2,:));      %曲线
                curve_distance2(i, :) = curve_data_distance2(1, :) + ...
                    (usFocusArray(1)-usFocusArray(2))/(usFocusArray(2)-usFocusArray(3))*(curve_data_distance2(1,:) - curve_data_distance2(2,:));  
            elseif (i > 1 && i < 7)
                curve_distance1(i, :) = curve_data_distance1(i-1, :);
                curve_distance2(i, :) = curve_data_distance2(i-1, :);
            elseif i == 7
                curve_distance1(i, :) = curve_data_distance1(5, :) + ...
                    (usFocusArray(7)-usFocusArray(6))/(usFocusArray(6)-usFocusArray(5))*(curve_data_distance1(5,:) - curve_data_distance1(4,:));      %曲线
                curve_distance2(i, :) = curve_data_distance2(5, :) + ...
                    (usFocusArray(7)-usFocusArray(6))/(usFocusArray(6)-usFocusArray(5))*(curve_data_distance2(5,:) - curve_data_distance2(4,:));       
            end
        end
        % 550℃曲线调整
        for i = 5000:usCurveTemperatureNumber
            curve_distance2(1,i) = curve_distance2(1,i)-(i-5000)*1.986;
            curve_distance2(2,i) = curve_distance2(2,i)-(i-5000)*1.864;
            curve_distance2(3,i) = curve_distance2(3,i)-(i-5000)*1.736;
            curve_distance2(4,i) = curve_distance2(4,i)-(i-5000)*1.563;
            curve_distance2(5,i) = curve_distance2(5,i)-(i-5000)*1.463;
            curve_distance2(6,i) = curve_distance2(6,i)-(i-5000)*1.361;
            curve_distance2(7,i) = curve_distance2(7,i)-(i-5000)*1.189;
        end
        % 20240620
        for i = 400:1200
            curve_distance2(1,i) = curve_distance2(1,i)+(i-400)*0.0225;
            curve_distance2(2,i) = curve_distance2(2,i)+(i-400)*0.02;
            curve_distance2(3,i) = curve_distance2(3,i)+(i-400)*0.01875;
        end
        for i = 1201:1700
            curve_distance2(1,i) = curve_distance2(1,i)+17+(i-1200)*0.032;
            curve_distance2(2,i) = curve_distance2(2,i)+16+(i-1200)*0.028;
            curve_distance2(3,i) = curve_distance2(3,i)+15+(i-1200)*0.024;
        end
        for i = 1701:2200
            curve_distance2(1,i) = curve_distance2(1,i)+33+(i-1700)*0.005;
            curve_distance2(2,i) = curve_distance2(2,i)+30+(i-1700)*0.004;
            curve_distance2(3,i) = curve_distance2(3,i)+27+(i-1700)*0.004;
        end
        for i = 2201:3200
            curve_distance2(1,i) = curve_distance2(1,i)+36+(i-2200)*0.037;
            curve_distance2(2,i) = curve_distance2(2,i)+32+(i-2200)*0.036;
            curve_distance2(3,i) = curve_distance2(3,i)+29+(i-2200)*0.035;
        end
        for i = 3201:usCurveTemperatureNumber
            curve_distance2(1,i) = curve_distance2(1,i)+73+(i-3200)*0.113;
            curve_distance2(2,i) = curve_distance2(2,i)+68+(i-3200)*0.109;
            curve_distance2(3,i) = curve_distance2(3,i)+64+(i-3200)*0.104;
        end

       for i = 1:usFocusArrayLength/2
         figure(3),hold on, plot(curve_distance1(i, :));
         figure(4),hold on, plot(curve_distance2(i, :));
         fwrite(fid, curve_distance1(i,:), 'uint16');
         fwrite(fid, curve_distance2(i,:), 'uint16');
       end
