...
curve_name = 'ita_param_Custom2.raw';
...
%     画曲线
    ...
    if transform_open == 1
        ...
        fid = fopen([[path '/Custom2'] '/' curve_name], 'w');  
        ...
        [a b c ] = xlsread('Y16记录表_定制范围2.xls');      
        m_data = a(1:2,2:6);        
%         m_data = cellfun(@str2num,m_data);
        ...
        %-10℃环温对应曲线     
        Data_10(1:4,1)=[70 90 150 200];    %Low
        Data_10(1:4,2)=(m_data(1,2:5))';  
        Data_10(:,2) =  Data_10(:,2) - Data_10(2,2) + 3000;
        nCurve(5,:) = Curve_Low(Data_10);

   
         %0℃环温对应曲线     
        Data_0(1:4,1)=[70 90 150 200];    %Low 
        Data_0(1:4,2)=(m_data(2,2:5))';  
        Data_0(:,2) =  Data_0(:,2) - Data_0(2,2) + 3000;
        nCurve(6,:) = Curve_Low(Data_0);
...
%         fwrite(fid, usFocusArray, 'uint16');     %焦温矩阵(数组) （10bytes）
        fwrite(fid, usFocusArray, 'int16');     %焦温矩阵(数组) （10bytes）有符号
...
function [nCurve] = Curve_Low(Data)
    ...
    AD_array = Data(:,2) ;
    T_array = Data(:,1) * 10;
    p = polyfit(T_array,AD_array, Order); %2次拟合
