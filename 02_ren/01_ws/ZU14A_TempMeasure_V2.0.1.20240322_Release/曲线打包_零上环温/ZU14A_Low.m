...
% path_curve_data= fullfile(path_lens,folder_names(k),'\Low'); %曲线包存放的路径
path_curve_data= fullfile(path_lens,folder_names(k)); %曲线包存放的路径
curve_name = 'ita_param_Low.raw'; %曲线包名
...
%     figure(2);
%     for i = 1:usFocusArrayLength/2
%         plot (1:2300,curve_data_distance05(i,:));hold on ;
%     end
...
%         [a b c ] = xlsread('Y16记录表_-20~150.xlsx');      
        data_excel_name =  'Y16记录表_-20~150.xlsx';
...
%             usFocusArray(5), 6500+800];    % 焦温数组
        ...
        %3℃环温对应曲线     
%         Data_3(1:5,1)=[70 90 110 150 200];    %Low
          Data_3(1:5,1)=[20    40    60   100   150]+50   %Low
        Data_3(1:5,2)=(m_data(1,4:8))';  
        Data_3(:,2) =  Data_3(:,2) - Data_3(1,2) + 3000;
        nCurve(2,:) = Curve_Low(Data_3);

   
         %13℃环温对应曲线     
        Data_13(1:6,1)=[50 70 90 110 150 200];    %Low 
        Data_13(1:6,2)=(m_data(2,3:8))';  
        Data_13(:,2) =  Data_13(:,2) - Data_13(2,2) + 3000;
        nCurve(3,:) = Curve_Low(Data_13);

        
        %23℃环温对应曲线
        Data_23(1:7,1)=[40 50 70 90 110 150 200];
        Data_23(1:7,2)=(m_data(3,2:8))';
        Data_23(:,2) =  Data_23(:,2) - Data_23(3,2) + 3000;
        nCurve(4,:) = Curve_Low(Data_23);

        %33℃环温对应曲线
        Data_33(1:6,1)=[50 70 90 110 150 200];
        Data_33(1:6,2)=(m_data(4,3:8))';  
        Data_33(:,2) =  Data_33(:,2) - Data_33(2,2) + 3000;
        nCurve(5,:) = Curve_Low(Data_33);
        
        %43℃环温对应曲线     
        Data_43(1:5,1)=[70 90 110 150 200];    %Low
        Data_43(1:5,2)=(m_data(5,4:8))';  
        Data_43(:,2) =  Data_43(:,2) - Data_43(1,2) + 3000;
        nCurve(6,:) = Curve_Low(Data_43);

        
        %扩容5℃以及65℃焦温曲线，避免-10℃环温以及50℃环温温度不准
